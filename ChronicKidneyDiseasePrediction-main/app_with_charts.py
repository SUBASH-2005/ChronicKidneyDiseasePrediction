...
def _patient_insights_df() -> pd.DataFrame:
    rows = [
        {"Metric": "Age", "Value": float(age), "Min": 0.0, "Max": 100.0},
        {"Metric": "Blood Pressure", "Value": float(bp), "Min": 40.0, "Max": 200.0},
        {"Metric": "Specific Gravity", "Value": float(sg), "Min": 1.005, "Max": 1.025},
        {"Metric": "Albumin", "Value": float(al), "Min": 0.0, "Max": 5.0},
        {"Metric": "Sugar", "Value": float(su), "Min": 0.0, "Max": 5.0},
        {"Metric": "Blood Glucose", "Value": float(bgr), "Min": 50.0, "Max": 500.0},
    ]
    df = pd.DataFrame(rows)
    df["Normalized (0-100)"] = ((df["Value"] - df["Min"]) / (df["Max"] - df["Min"]) * 100.0).clip(0, 100)
    return df
...
<div class="card fade-in" style="margin-top: 12px;">
  <div class="card-title">
    <div class="left">
      <div class="icon">📊</div>
      <h2>Patient Snapshot Chart</h2>
    </div>
  </div>
  <div class="subtle">A normalized view of the entered metrics (0–100) to quickly spot outliers.</div>
</div>
        """,
        unsafe_allow_html=True,
    )
    snap = _patient_insights_df().set_index("Metric")[["Normalized (0-100)"]]
    st.bar_chart(snap, height=240)
...
<div class="card fade-in" style="margin-top: 10px;">
  <div class="rings">
    <div class="ring-card">
      <div class="ring-wrap">
        <div class="ring" style="--p:{ckd_prob:.2f}; --c:{ALERT_RED};"></div>
    ...
    <div class="ring-card">
      <div class="ring-wrap">
        <div class="ring" style="--p:{healthy_prob:.2f}; --c:{SECONDARY};"></div>
